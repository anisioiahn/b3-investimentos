...
def api_builder_executar():
    ...
    try:
        ...
        with conn.cursor(cursor_factory=pex.RealDictCursor) as cur:
            ...
            # Filtro de risco via subquery no WHERE
            if risco_max > 0:
                conditions.append(f"""
                    COALESCE((
                        ...
                        LEFT JOIN financial_snapshots fs2
                            ON fs2.asset_id = ms2.asset_id
...
def obter_valor_atual_carteira_geral(usuario_id, categoria_id='TODAS', ticker=None):
    ...
    for p in posicoes:
        ...
        preco = preco_atual if preco_atual is not None else p.get("preco_medio", 0)
        total += (preco or 0) * (p.get("quantidade") or 0)
    return total
