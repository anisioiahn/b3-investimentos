    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            if results and results[0].get("historicalDataPrice"):
                ...
                return [{"date": h.get("date"), "close": h.get("close"), "open": h.get("open"), "high": h.get("high"), "low": h.get("low"), "volume": h.get("volume")} for h in hist if h.get("close")]
    except Exception as e:
...
def main():
    print("Buscando cotações — 1 ativo por vez (plano gratuito brapi.dev)...")
    dados = buscar_todas_cotacoes()
    total = sum(len(s["empresas"]) for s in dados["setores"].values())
    com_preco = sum(1 for s in dados["setores"].values() for e in s["empresas"] if e.get("preco"))
    print(f"\n✅ {com_preco}/{total} empresas com cotação")
